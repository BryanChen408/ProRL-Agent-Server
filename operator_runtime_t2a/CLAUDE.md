...
> **打包骨架(推荐起点)**: `.claude/workflows/templates/kernel_skeleton/` 是一份
> **和判分链路实测兼容的自包含模板**(CMakeLists / register.cpp / setup.py /
> model_new 顶部加载逻辑 + elementwise kernel 骨架)。`cp -r` 复制成 `{op_name}/` 后,
> **只改 kernel 的 `Compute()` 数学,打包四件套原样保留**。三条硬规则:①扩展名用扁平
> `{op_name}_ext`(别用 `{op_name}._C`);②model_new 顶部「import 失败 →
> `torch.ops.load_library`」兜底别删(register.cpp 无 PYBIND11_MODULE,裸 import 必失败);
> ③注册走 TORCH_LIBRARY + load_library,别走 import/wheel 路线。用它可避免
> op_not_registered / 打包类失败,把预算留给真正的 kernel 数学。
