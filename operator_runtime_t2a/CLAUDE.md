...
> ops.h / op_host / op_kernel / utils)全部按 op_name 预填好、命名自洽,**不要重写这些打包件**。
> 你**只需**:① 在 `kernel/op_kernel/{op_name}_kernel.cpp` 的 `Compute()` 里写算子数学;
> ② 在 `op_host/{op_name}.cpp` 填 tiling/启动参数;③ 在 `model_new_ascendc.py` 的 forward
> 填对调用(真调 `torch.ops.npu.{op_name}`,别加 plain-torch 兜底)。三条硬规则:①扩展名
> 扁平 `{op_name}_ext`(别用 `{op_name}._C`);②model_new 顶部「import 失败 →
> `torch.ops.load_library`」兜底别删(register.cpp 无 PYBIND11_MODULE,裸 import 必失败);
> ③注册走 TORCH_LIBRARY + load_library,别走 import/wheel 路线。模板原件在
> `.claude/workflows/templates/kernel_skeleton/` 可查。
