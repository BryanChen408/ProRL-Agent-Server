...
def _copy_tree(src: Path, dst: Path) -> None:
    ...
    shutil.copytree(src, dst, symlinks=True)
                                    ...
                                    # <workdir>/plugins-community/<plugin>/skills → .claude/skills
_PLUGIN_NAME = "tilelang2ascendc-ops-generator"
...
def _link_upstream_skills_path(workdir: Path) -> None:
    ...
    CLAUDE.md 是官方 `agents/tilelang2ascendc-kernel-generator.md` 的**逐字拷贝**,里面对
    skill 路径有**两种写法并存**:

    - 10 处 `.claude/skills/tilelang2ascend-*/…`  ← 在本环境成立
    - 2 处 `plugins-community/tilelang2ascendc-ops-generator/skills/…`  ← **不成立**
      (第 182 行「Skill 参考资料」清单前缀,以及 Phase 1.2 那条
       `cp …/csrc/utils/torch_kernel_helper.h` —— 每个 session 必踩,
       而该头文件是 kernel 编译必需的)

    pin 版(cannbot `ascend-kernel-developer.md`)是同一个病、不同的错路径
    (`ops-lab/tilelang-to-ascendc/skills/`),实测 agent 因此 ≥20 次去找不存在的目录、
    另有 3 次 `ops-lab not found`。**上游换了布局但没有对账,所以问题跟着换了个前缀延续。**

    修在这里而不是改 CLAUDE.md:铁律 2 要求逐字派生,改文档等于跟上游分叉;
    且同类路径不止这两处 —— 补一条 symlink 让两种写法同时成立,一次覆盖全部。

    相对链接(从 `<workdir>/plugins-community/<plugin>/` 上跳两级即 workdir),
