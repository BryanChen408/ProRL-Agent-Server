...
def _answer_from_trajectory(trajectory: Trajectory) -> str | None:
    ...
    operator_samples path: vime puts ``answer`` in ``sample.metadata``;
    ``operator_profile.py:194`` stores it under ``request.metadata['sample_metadata']``;
    ``pipeline.py:502`` lifts ``request.metadata`` into ``trajectory.metadata['task_metadata']``.
    So the answer lives at ``task_metadata.sample_metadata.answer``. Agent only sees
    ``instruction`` → no leak.
    """
    def _dig(md: dict[str, Any]) -> str | None:
        ...
        if isinstance(tm, dict):
            # operator_samples 把 sample.metadata 存进 task_metadata.sample_metadata
            # (operator_profile.py:194 → pipeline.py:502);答案 = sample_metadata.answer(judge-only)。
            sm = tm.get("sample_metadata")
            if isinstance(sm, dict) and sm.get("answer") is not None:
                return str(sm["answer"])
            if tm.get("answer") is not None:  # 兼容:答案直接放 task_metadata
                return str(tm["answer"])
        if md.get("answer") is not None:  # 兼容:答案直接放顶层
