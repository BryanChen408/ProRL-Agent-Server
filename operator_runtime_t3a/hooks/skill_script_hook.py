...
def _npu_wrap_env(command: str) -> dict:
...
# [R8] 直接形态的 NPU 脚本调用外层包租约执行器:R3 注入的 NPU_WRAP 只有
...
def execute_intercepted(command: str) -> None:
    ...
    exit_code, stdout, stderr, duration_ms = _run_command(_lease_wrap_command(command), cwd)
