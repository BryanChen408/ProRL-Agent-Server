...
6. **算子类别经验文件**（若存在）：`{project_root}/.claude/memory/kernel-opt-{category}.md`。该文件包含经过验证的 **Layer 1 设计约束**（硬性规则）。若其 Layer 1 约束与传入的 `sketch` 冲突，**必须以 Layer 1 约束为准**修正代码架构，不得盲目遵循一个已知劣化的草图。
