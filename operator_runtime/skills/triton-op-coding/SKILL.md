...
- **硬件规格**（每次生成都必须加载）：
 `@/opt/canonical/skills/npu-arch/references/npu-arch-guide-triton.md` 和 `@/opt/canonical/skills/npu-arch/references/npu-hardware-params.md`
...
6. 写入 `output/submission/{op_name}_impl.py` 后立即停止本轮代码生成，下一步交给 `triton-op-verifier`
...
6. 写入修复后的 `output/submission/{op_name}_impl.py` 后立即停止，下一步交给 `triton-op-verifier` 复测
