...
def _instruction(op_name: str, model_rel: str) -> str:
    ...
    return (
        ...
        f"(class Model; inputs from get_inputs() or get_input_groups(), with get_init_inputs() when defined). Complete the pre-generated project directory `{op_name}/` "
        "in place.\n\n"
        ...
        "project. Preserve the build and loader mechanisms, but treat output allocation, dtype checks, "
        "tiling and identity-copy computation as placeholders, not the operator specification. Read the "
        "full reference, including __init__, forward and relevant helpers; implement all computation "
        "steps, parameters, branches and outputs rather than inferring semantics from the operator name.\n\n"
        ...
        "for a separate project-initialization workflow. In this session, use Read to consult the existing "
        "cannbot design and API resources named by CLAUDE.md, then implement and self-review in the same "
        "main trajectory. Do not invoke Skill or dispatch extra agents. Skipping TileLang does not skip "
        "the brief design and API checks. Use SOC_VERSION from the environment; do not override or guess it.\n\n"
        ...
        "the first implementation is ready, then once per effective repair batch within the reported "
        "budget. Do not evaluate an untouched skeleton merely because metrics do not exist yet. If a "
        "previous evaluation is still running, collect its result rather than launching another; "
        "do not repeat evaluation of unchanged source.\n\n"
        "Rules:\n"
        ...
        "While task_complete=false, follow next_step to repair or optimize within the remaining budget; "
