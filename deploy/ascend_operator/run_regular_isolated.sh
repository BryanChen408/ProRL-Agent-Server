...
RUN_ID=${RUN_ID:-regular_isolated_$(date +%Y%m%d-%H%M%S)}
...
echo "[stage] launch regular training RUN_ID=${RUN_ID} LOG_FILE=${LOG_FILE}"
exec bash "${SLIME_ROOT}/scripts/ascend_script/run-polar-e2e-regular-qwen36-35b-a2.sh"
