...
**改法**:把"为什么这么写"的论证**全部移出脚本**,留到 `SESSION_AUDIT` / `HANDOFF` /
commit message 里(那些 agent 读不到)。脚本里只留必要的机械说明
(参数含义、单位、调用契约),不写"这条防的是什么攻击"。

**连指针注释也不留** —— 不要写「别改,理由见 MIGRATION_T2A.md」这种。
理由:① 它同样是人工痕迹;② 它反而**标记出了敏感位置**,等于告诉模型"这行有讲究,
值得研究" —— 比不写更糟。约束靠**代码本身**保证(写死常量、不给 env 开关),
而不是靠注释劝阻;防回归靠 **CI/preflight 断言**,不靠留言。

⚠️ 相应地,`--repeats 1` / `--device` 那类约束要么写死成不可配置的字面量,
要么加一条 preflight 校验(例如断言 pipeline 里不出现 `--compare` / `--repeats [2-9]`),
把"别改"从注释变成机械保证。
