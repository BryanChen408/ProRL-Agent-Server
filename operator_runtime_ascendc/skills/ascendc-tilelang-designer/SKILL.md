---
name: ascendc-tilelang-designer
description: >
...
**简单算子**（Elementwise, Index 等）走 `ascendc-case-simplifier` → `ascendc-code-gen` 路径，不使用本 skill。
...
- `@scripts/evaluate_tilelang.sh` — TileLang 评测脚本（当前仅供可选调试，不作为流程 gate）
...
3. `TileLang 自检（可选）`
   如用户明确要求，或为了排查 DSL 语法 / 编译问题，可调用 `@scripts/evaluate_tilelang.sh {output_dir}` 做辅助检查；但 TileLang 结果当前不作为 correctness gate，也不作为性能测试输入。若遇到框架语义缺陷、尾块处理异常或其他 TileLang 自身 bug，应保留设计表达并在最终说明中明确记录，不要为了通过 TileLang 验证而扭曲设计。
