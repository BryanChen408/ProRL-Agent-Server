...
# Canonical source: ascendc-translator/scripts/verification_ascendc.py
_ASCENDC_SCRIPTS = (
    Path(__file__).resolve().parents[1] / "ascendc-translator" / "scripts"
)
if str(_ASCENDC_SCRIPTS) not in sys.path:
    sys.path.insert(0, str(_ASCENDC_SCRIPTS))

from verification_ascendc import (  # noqa: E402
    _load_module,
    _find_model_class,
    _clone_value,
    _move_to_device,
...
def main():
    if len(sys.argv) != 2:
        logger.error("Usage: python .claude/skills/ascendc-tilelang-designer/scripts/verification_tilelang.py <op>")
