...
find_workdir() {
  ...
  while [[ "${candidate}" != "/" ]]; do
    if [[ -f "${candidate}/.claude/skills/ascendc-tilelang-designer/scripts/verification_tilelang.py" ]]; then
      echo "${candidate}"
      return 0
    fi
...
WORKDIR="$(find_workdir)" || {
  echo "Unable to locate repository root containing .claude/skills/ascendc-tilelang-designer/scripts/verification_tilelang.py" >&2
  exit 1
}
...
usage() {
  cat <<'EOF'
Usage: bash skills/ascendc/ascendc-tilelang-designer/scripts/evaluate_tilelang.sh [task]
...
Examples:
  bash <path-to-ascendc-tilelang-designer>/scripts/evaluate_tilelang.sh
  bash <path-to-ascendc-tilelang-designer>/scripts/evaluate_tilelang.sh matmul_add
  SSH_TARGET=ascend-box bash <path-to-ascendc-tilelang-designer>/scripts/evaluate_tilelang.sh matmul_add
  REMOTE_EVAL_WORKDIR=workdir_remote_eval_wzz bash <path-to-ascendc-tilelang-designer>/scripts/evaluate_tilelang.sh current_task
  REMOTE_BASE_DIR=/data/eval bash <path-to-ascendc-tilelang-designer>/scripts/evaluate_tilelang.sh current_task
  ASCEND_RT_VISIBLE_DEVICES=3 bash <path-to-ascendc-tilelang-designer>/scripts/evaluate_tilelang.sh current_task
EOF
}
...
if [[ ! -f "${WORKDIR}/.claude/skills/ascendc-tilelang-designer/scripts/verification_tilelang.py" ]]; then
  echo "Missing verification script: ${WORKDIR}/.claude/skills/ascendc-tilelang-designer/scripts/verification_tilelang.py" >&2
  exit 1
fi
...
if python -c 'import tilelang; import torch; import torch_npu' >/dev/null 2>&1; then
  ...
  PYTHONPATH="${PYTHONPATH_PREFIX}${PYTHONPATH:+:${PYTHONPATH}}" \
    ...
    python .claude/skills/ascendc-tilelang-designer/scripts/verification_tilelang.py "${TASK_DIR}"
  exit 0
fi
...
ASCEND_RT_VISIBLE_DEVICES="${ASCEND_RT_VISIBLE_DEVICES}" \
python .claude/skills/ascendc-tilelang-designer/scripts/verification_tilelang.py "${TASK}"
'
EOF
