...
def _find_custom_opp_path(base: Path) -> tuple[Path | None, str | None]:
    """查找 AscendC 自定义算子路径，按优先级: installed → build → cpack."""
    candidate = base / "vendors" / "customize"
    if candidate.exists():
        return candidate, "installed_vendors"

    for item in base.iterdir():
        if item.is_dir() and item.name.lower().endswith('custom'):
            cand = item / "vendors" / "customize"
            if cand.exists():
                return cand, "build_custom_vendors"
    ...
    for item in base.iterdir():
        if item.is_dir() and item.name.lower().endswith('custom'):
            ...
            if run_pkg.exists():
                return run_pkg, "cpack_run_package"

    return None, None


def _setup_library_path(custom_opp: Path, existing_ld: str) -> str:
    """构建 LD_LIBRARY_PATH，追加 torch、torch_npu、op_api 库路径."""
    paths_to_add = []
    ...
    try:
        ...
        logger.debug("torch lib not found for LD_LIBRARY_PATH")
    ...
    try:
        ...
        logger.debug("torch_npu lib not found for LD_LIBRARY_PATH")
...
def setup_ascend_environment(op_name: str, output_path: str) -> str | None:
    ...
    custom_opp, found_in = _find_custom_opp_path(base)

    if custom_opp and custom_opp.exists():
        os.environ["ASCEND_CUSTOM_OPP_PATH"] = str(custom_opp)
        existing_ld = os.environ.get("LD_LIBRARY_PATH", "")
        os.environ["LD_LIBRARY_PATH"] = _setup_library_path(custom_opp, existing_ld)
        return found_in
    return None
...
class OperatorTypeDetector:
    ...
    @staticmethod
    def _infer_reduction_axis(attributes: dict, shape_info: dict) -> dict | None:
    ...
    @staticmethod
    def _infer_from_name(op_name: str) -> str:
        name = op_name.lower()
        rules = [
    ...
    @staticmethod
    def _try_read_op_desc(output_path: str, op_name: str) -> tuple:
        """从 op_desc.json 读取算子类型信息，返回 (op_type, source, attributes, shape_info)。"""
        ...
        try:
            ...
            return op_type, "op_desc.json", desc.get("attributes", {}), desc.get("shape_info", {})
        except (json.JSONDecodeError, KeyError):
            return "unknown", "none", {}, {}

    @staticmethod
    def _try_read_project_json(output_path: str, op_name: str) -> tuple:
        """从 project.json 读取算子类型，返回 (op_type, source)。"""
        ...
        try:
            ...
            if isinstance(proj, list) and len(proj) > 0:
                return proj[0].get("op", "unknown").lower(), "project.json"
            if isinstance(proj, dict):
                return proj.get("op_type", "unknown").lower(), "project.json"
            return "unknown", "none"
        except (json.JSONDecodeError, KeyError):
            return "unknown", "none"
    ...
    def detect(self, op_name: str, output_path: str) -> dict:
        ...
        # 1. 从 op_desc.json 读取 (优先级最高)
        op_type, source, attributes, shape_info = self._try_read_op_desc(output_path, op_name)

        # 2. fallback: 从 project.json 读取
        if op_type == "unknown":
            op_type, source = self._try_read_project_json(output_path, op_name)
...
class DiffAnalyzer:
    ...
    @staticmethod
    def _basic_stats(golden, actual, abs_diff, mismatch_mask) -> dict:
    ...
    @staticmethod
    def _error_distribution(golden, actual, abs_diff) -> dict:
    ...
    @staticmethod
    def _value_range(golden, actual) -> dict:
        def _s(arr, name):
            return {
    ...
    @staticmethod
    def _check_tail_spike(mm, shape):
        ld = shape[-1]
    ...
    @staticmethod
    def _check_dim_concentration(mm, shape):
        for dim, dim_size in enumerate(shape):
            ...
            if np.max(ra) > 0.5 and np.min(ra) < 0.1:
                ...
                return {"pattern": "dimension_concentration", "confidence": 0.8,
                        "evidence": f"dim={dim}(size={dim_size}), 索引 {bad} mismatch 偏高",
                        "detail": {"dim": dim, "bad_indices": bad,
                                   "rates": [round(r, 4) for r in rates]}}
        return None

    @staticmethod
    def _check_magnitude_correlation(golden, mm):
    ...
    @staticmethod
    def _check_boundary_concentration(mm, shape):
        if len(shape) < 2:
            return None
        tm = int(np.sum(mm))
        if tm == 0:
        ...
        for dim, dim_size in enumerate(shape):
            if dim_size <= 2:
                continue
            for edge in [0, dim_size - 1]:
                s = tuple([slice(None)] * dim + [edge] + [slice(None)] * (len(shape) - dim - 1))
                bm += int(np.sum(mm[s]))
                bt += mm[s].size
        boundary_majority = bm / tm > 0.6
        boundary_concentration = bm / bt > 0.2
        if bt > 0 and boundary_majority and boundary_concentration:
    ...
    @staticmethod
    def _worst_elements(abs_diff, golden, actual, top_k=10):
    ...
    @staticmethod
    def _tail_analysis(abs_diff, mm, shape):
        ld = shape[-1]
        results = {}
        for ts in [8, 16, 32, 64, 128, 256]:
            tl = ld % ts
    ...
    @staticmethod
    def _dimension_analysis(abs_diff, mm, shape):
        analysis = []
        for dim, dim_size in enumerate(shape):
            if dim_size <= 1:
                continue
            ...
            analysis.append({
                ...
                "per_index_rates": [round(r, 4) for r in rates] if dim_size <= 64 else None,
            })
        return analysis
...
class IntermediateProbe:
    ...
    @staticmethod
    def probe(op_name: str, output_path: str) -> dict | None:
        return None
...
class CodeMapper:
    ...
    @staticmethod
    def map(worst_elements: list, kernel_path: str) -> dict | None:
        return None
...
class PrecisionForensics:
    ...
    def run(self) -> dict:
        try:
            ...
            final, worst = self._build_report(data, op_type_info)
            ...
            self._log_summary(final, op_type_info, worst, report_path)
            return final
    ...
    def _build_report(self, data: dict, op_type_info: dict) -> tuple[dict, dict]:
        """Analyze outputs and build the forensics report dict."""
    ...
    def _log_summary(self, final: dict, op_type_info: dict, worst: dict,
                     report_path: str) -> None:
        """Log forensics summary to logger."""
        stats = worst["basic_stats"]
        trend = final.get("history_trend")
        l5 = final.get("L5_intermediate")
        l7 = final.get("L7_code_mapping")
        logger.info(f"精度取证完成 (attempt={self.attempt})")
        logger.info(f"  op_type: {op_type_info['op_type']} (source={op_type_info['source']})")
        logger.info(f"  primary_hint: {final['primary_hint']} "
                    f"(confidence={final['primary_confidence']:.2f})")
        logger.info(f"  evidence: {final['primary_evidence']}")
        logger.info(f"  mismatch: {stats['mismatch_ratio']:.2%} "
                    f"({stats['num_mismatched']}/{stats['total_elements']})")
        logger.info(f"  max_diff: {stats['max_abs_diff']:.6f}")
        logger.info(f"  L5: {'available' if l5 else 'not available (TODO)'}")
        logger.info(f"  L7: {'available' if l7 else 'not available (TODO/Agent)'}")
        if trend:
            logger.info(f"  趋势: {'↓ 改善中' if trend['mismatch_improving'] else '↑ 未改善'}")
