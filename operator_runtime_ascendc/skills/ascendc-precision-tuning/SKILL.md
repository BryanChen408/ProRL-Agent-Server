---
name: ascendc-precision-tuning
description: >
  AscendC 算子精度调优的方法论参考(构造式审计)。当 ascend-kernel-developer
  Phase 4 Conductor 判定为 D 类(精度不匹配)时，agent 必须调用本 skill
  进行取证→审计→修复分析，将输出作为本轮 ac_conductor_suggestion 的修复依据。
  也适用于常规 Conductor 修复建议无法收敛时的深度根因分析。
argument-hint: >
  agent 在 D 类精度失败时调用本 skill：读 SKILL.md 学习审计思路、查
  references/precision_knowledge_base.json 找匹配 pattern、参考
  references/decomposition_examples/ 看算子分解，将根因诊断和修复建议
  作为本轮 ac_conductor_suggestion。
---

## 约束声明(必读)

**D 类(精度不匹配)触发时 agent 必须调用本 skill** 进行取证→审计→修复分析,将其输出的根因诊断和修复建议作为本轮 `ac_conductor_suggestion`。也适用于常规 Conductor 修复建议无法收敛时的深度根因分析。ascend-kernel-developer 在其他情况下应优先按自身 4.1-4.4 主流程判断。
...
ascend-kernel-developer Phase 4 报精度类失败(MERE 或 MARE 超阈),且 Conductor 给出的修复建议在最近一轮 outcome 为 stagnant 或 regressed —— 此时可查阅本 skill 的 SKILL.md 方法论 / `references/precision_knowledge_base.json` / `references/decomposition_examples/`。agent 可以完全自行决定是否采纳。
...
在 Sub-step 2.3 AscendC 实现对照时，逐项对照此表:
...
| **BF16 通过但 FP16/FP32 失败** | API不支持BF16走fallback / 精度阈值差异 | 先排查API fallback分支, 再检查阈值适配 |
| **FP32 通过但 FP16/BF16 失败** | 半精度中间计算精度不足 | 升精度: Cast→FP32计算→Cast回半精度 |
...
python3 skills/ascendc-evaluation/scripts/generate_pybind.py \
...
4. `skills/ascendc-precision-tuning/references/decomposition_examples/README.md` — 分解示例索引
...
1. 根据 `L8_operator.op_type` 定位对应的 lowering 示例 (必须读取完整文件):
   - `softmax` → `skills/dsl-lowering/references/lowering_examples/softmax/softmax_process_example_for_lowering.md`
   - `mse_loss` / loss 类 → `skills/dsl-lowering/references/lowering_examples/mse_loss/mse_loss_process_example_for_lowering.md`
   - `average_pooling2d` / pooling 类 → `skills/dsl-lowering/references/lowering_examples/pooling/pooling_process_example_for_lowering.md`
   - `leaky_relu` / 逐元素类 → `skills/dsl-lowering/references/lowering_examples/leaky_relu/leaky_relu_process_example_for_lowering.md`
   - 无精确匹配 → 读取最接近的示例 (优先选同类计算模式)
2. **必须读取**: `skills/dsl-lowering/references/error_correction/error_correction_examples.md`
3. **必须读取**: `skills/dsl-lowering/references/lowering_examples/non_aligned/non_aligned_example_for_lowering.md`
4. **必须读取**: `skills/dsl-lowering/references/ascend_api/tl_asc_routing.md` — AscendC API 权威参考
...
| **GlobalTensor.SetValue** | 输出全为0 | 改用 LocalTensor.SetValue + DataCopyPad ⭐⭐⭐ |
...
```bash
source "{output_path}/vendors/customize/bin/set_env.bash" && \
python3 skills/ascendc-evaluation/scripts/evaluate.py \
    {op_name} --output-path "{output_path}"
```
...
## 诊断模式 A: FP32 通过但 FP16/BF16 失败

最常见的精度诊断信号：FP16/BF16 通常共享相同的 Cast-to-FP32 计算路径（`if constexpr (std::is_same_v<T, bfloat16_t> || std::is_same_v<T, half>)`），FP32 通过说明核心算法正确，问题在半精度转换环节。
...
## 诊断模式 B: BF16 通过，但 FP16/FP32 失败

可能由两类原因导致：

**原因1 — API 不支持导致 fallback 路径差异**：部分 AscendC 算术/归约 API 不支持 BF16，开发者为 BF16 实现更简单的 fallback 路径。BF16 fallback 通过说明逻辑正确，问题出在 FP16/FP32 路径中调用的复杂 API 上。

**原因2 — 精度阈值/数值范围差异**：BF16（mantissa 7bit，指数范围同 FP32）与 FP16（mantissa 10bit，指数范围更小）精度特性不同：BF16 不易溢出但尾数精度低，FP16 易溢出但尾数精度高。
...
| 方法 | 场景 | 操作 |
|------|------|------|
| **对比法** | 存在正常工作的参考代码 | 逐行对比差异，定位分叉点 |
| **边界二分法** | 误差集中在特定参数范围 | 记录通过/失败的临界点，分析分支选择 |
| **数值验证法** | 怀疑公式或常量错误 | 用固定输入值手动计算期望输出，与 kernel 实际输出对照 |
| **Identity 测试** | 不确定数据搬运是否正确 | 将 kernel 改为 CopyIn→CopyOut 直通，验证 DataCopy/EnQue/DeQue |
| **逐算子增量测试** | 多步计算中不确定哪步出错 | 从 identity 出发，每次只加一个运算（Mul→Add→Sub），逐次验证 |
