...
# 用法:
#   # 单 NPU 模式
#   bash .claude/skills/ascendc-performance-analyzer/script/run_benchmark_ascendc.sh --benchmark-dir /path/to/KernelBench --level 1 --range 41-53 --npu 0 --output /path/to/output
#
#   # 多 NPU 并行模式
#   bash .claude/skills/ascendc-performance-analyzer/script/run_benchmark_ascendc.sh --benchmark-dir /path/to/KernelBench --level 1 --range 1-30 --npu-list "0,1,2,3,4,5" --output /path/to/output
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            echo "用法: bash .claude/skills/ascendc-performance-analyzer/script/run_benchmark_ascendc.sh --benchmark-dir <path> --level <N> [--range <start-end> | --ids <id_list>] [--npu <id> | --npu-list <list>] --output <path>"
            echo ""
            echo "参数:"
            ...
            echo "示例:"
            echo "  # 单 NPU 串行模式"
            echo "  bash .claude/skills/ascendc-performance-analyzer/script/run_benchmark_ascendc.sh --benchmark-dir /path/to/KernelBench --level 1 --range 1-30 --npu 0 --output /path/to/output"
            echo ""
            echo "  # 多 NPU 并行模式"
            echo "  bash .claude/skills/ascendc-performance-analyzer/script/run_benchmark_ascendc.sh --benchmark-dir /path/to/KernelBench --level 1 --range 1-30 --npu-list \"0,1,2,3,4,5\" --output /path/to/output"
            exit 0
            ;;
        *) echo "未知参数: $1"; exit 1 ;;
    esac
...
echo "================================================================"
# 请根据你的文件实际路径调整下面这一行
python3 .claude/skills/ascendc-performance-analyzer/script/generate_report_dynamic.py -i "$OUTPUT_DIR" -o "$OUTPUT_DIR/final_batch_report.md"
