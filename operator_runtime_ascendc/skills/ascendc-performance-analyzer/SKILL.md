...
- `@scripts/performance.py` — 性能测试脚本，支持对比多种实现
...
2. **执行性能测试**
   调用 `@scripts/performance.py` 脚本进行性能测试。默认对比 `reference` 和 `ascendc`；只有在用户明确要求时才额外纳入 `tilelang`：
   ```bash
   python3 @scripts/performance.py --output_dir {output_dir} --output {output_dir}
   ```
